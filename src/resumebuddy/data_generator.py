    ...
    def _extract_json(self, text: str) -> Dict[str, Any]:
        ...
        if match:
            ...
            try:
                ...
                # If standard loads fails, attempt to clean common LLM junk (like raw newlines in strings)
                try:
...
"""
    ...
    async def run(self, count: int = 1000, output_file: str = "training_data.jsonl"):
        """
        Loops through industries and saves to JSONL incrementally.
        """
        print(f"Starting generation of {count} examples using {self.model_id} (Free Tier)...")
...
async def main():
    ...
    await generator.run(count=1000)
